...
import cv2
'''
Before running this script, node graph creation is  taken care of using another script which is part of this folder
Next, PBR node setup is already done manually
How? click on Principled BSDF -> click Ctrl + Shift + T
then for each principled bsdf, add a bump node, add output of normal map node to normal input of the principled bsdf
also add output of displacement node as height input to the bump node
then link bump node output to the Normal node input of the principled 
...
def annotate_graph(upper_name,lower_name):
    ...
    scene.render.filepath = '/home/supratik/Pictures/blender_progression/data_set/'+'ann1'+'.png'
    bpy.ops.render.render(write_still = 1)
    link(white_bsdf.outputs[0],mix_shader.inputs[2])
    link(black_bsdf.outputs[0],mix_shader.inputs[1])
    scene.render.filepath = '/home/supratik/Pictures/blender_progression/data_set/'+'ann2'+'.png'
    bpy.ops.render.render(write_still = 1)
    image1 = cv2.imread('/home/supratik/Pictures/blender_progression/data_set/ann1.png')# having the upper part as white
    image2 = cv2.imread('/home/supratik/Pictures/blender_progression/data_set/ann2.png')# having the lower part as white
    #upper annotation = image1 - image2
    #lower annotation = image2 - image1
    #image1 = white above black
    #image2 = black above white
    cwd = os.getcwd()
    os.chdir('/home/supratik/Pictures/blender_progression/data_set')
...
for i in range(dir_len-1):
    for j in range(i+1,dir_len):
        ...
        scene.render.image_settings.file_format = 'PNG'
        scene.render.filepath = '/home/supratik/Pictures/blender_progression/data_set/'+directories[i]+'_'+directories[j]+'_v_2.png'
        bpy.ops.render.render(write_still = 1)             
        print('rendered:{} and {}'.format(directories[i],directories[j]))
        time.sleep(10)
        #both above and lower lines are optional, written here to prevent program from aborting especially when RAM is low.
