...
def get_materials(directory_name,part):
    ...
    returns null
    '''
    base_material = bpy.data.materials['base_material2.017']
    ...
    metal_files = glob.glob('*metal*')
    mi_file = None
    if metal_files!=[]:
        mi_file = glob.glob('*metal*')[0] 
    os.chdir(cwd)
    ...
    '''
    if mi_file!=None:
        metal = principled_bsdf.inputs[4].links[0].from_node
        metal_img = bpy.data.images.load(filepath = '/home/supratik/Pictures/blender_progression/textures/'+directory_name+'/textures/'+mi_file)
...
for i in range(len(directories)):
    ...
    scene.render.image_settings.file_format = 'PNG'
    scene.render.filepath = '/home/supratik/Pictures/blender_progression/data_set/'+str(i)+'_'+'transparent_new.png'
    bpy.ops.render.render(write_still = 1)             
    print('rendered:')
    print(directories[i])
    #    print(directories[j])
